...
`k8s/manifest.yaml` define dos Deployments y un Service:
...
Cada versión sale de su rama. El `.` del final es el contexto de build.
...
minikube no ve el daemon local. Hay que meterle las imágenes, y quedan bajo el prefijo
`localhost/` —por eso el manifest pide `localhost/mdconverter:v1` y `:v2`.

```bash
minikube image load mdconverter:v1
minikube image load mdconverter:v2
minikube image ls | grep mdconverter   # localhost/mdconverter:v1 y :v2
```

### 3. Aplicar el manifest

El manifest no trae Namespace. Creá uno y dejalo como default de `kubectl` antes de aplicar,
así el resto de los comandos van sin `-n`:

```bash
kubectl create namespace mdconverter
kubectl config set-context --current --namespace=mdconverter
kubectl apply -f k8s/manifest.yaml
kubectl rollout status deploy/mdconverter-blue
kubectl rollout status deploy/mdconverter-green
```

Para volver al namespace anterior: `kubectl config set-context --current --namespace=default`.
...
```bash
kubectl port-forward deploy/mdconverter-green 8080:8000
kubectl port-forward deploy/mdconverter-blue  8081:8000
```
...
```bash
kubectl patch svc web -p '{"spec":{"selector":{"version":"green"}}}'   # pasar a green
kubectl patch svc web -p '{"spec":{"selector":{"version":"blue"}}}'    # volver a blue
kubectl describe service web | grep Selector                           # ver el color activo
```
...
kubectl delete namespace mdconverter
```
